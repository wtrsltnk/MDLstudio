...
void StudioModel::CalcBoneQuaternion(
    int frame,
    float s,
    mstudiobone_t *pbone,
    mstudioanim_t *panim,
...
void StudioModel::CalcBonePosition(
    int frame,
    float s,
    mstudiobone_t *pbone,
    mstudioanim_t *panim,
...
mstudioanim_t *StudioModel::GetAnim(
    mstudioseqdesc_t *pseqdesc)
{
...
float StudioModel::AdvanceFrame(
    float dt)
{
    mstudioseqdesc_t *pseqdesc;
...
// All rights reserved.
//////////////////////////////////////////////////////////////////////
void StudioModel::DrawIcosahedron(
    float x,
    float y,
    float z,
    float size)
{
    ...
    GLfloat Vertices[12][3] = {
        {-a, 0.0, b}, {a, 0.0, b}, {-a, 0.0, -b}, {a, 0.0, -b}, {0.0, b, a}, {0.0, b, -a}, {0.0, -b, a}, {0.0, -b, -a}, {b, a, 0.0}, {-b, a, 0.0}, {b, -a, 0.0}, {-b, -a, 0.0}};
...
void StudioModel::DrawHitboxes()
{
    ...
    for (i = 0; i < m_pstudiohdr->numhitboxes; i++)
    {
        ...
        if (i == m_selhitbox)
            glColor4f(0.7f, 0.0f, 0.2f, 0.8f);
        else if (pbboxes[i].bone == m_selbone)
            glColor4f(0.8f, 0.4f, 0.2f, 0.8f);
        else
            glColor4f(0.0f, 0.6f, 0.2f, 0.3f);
...
================
*/
void StudioModel::DrawAtachments()
{
    int i;
        ...
        if (ptexture[pskinref[pmesh->skinref]].flags & STUDIO_NF_CHROME)
        {
            while (i = *(ptricmds++))
            {
                ...
                for (; i > 0; i--, ptricmds += 4)
                {
                    ...
                    glColor4f(lv[0], lv[1], lv[2], fAlpha);
...
long StudioModel::GetPolyCount()
{
    return m_polycount;
}

mstudiotexture_t *StudioModel::GetMaterial(
    const char *name)
{
    mstudiotexture_t *pTextures;

    pTextures = (mstudiotexture_t *)((byte *)m_pstudiohdr + m_pstudiohdr->textureindex);
    for (int i = 0; i < m_pstudiohdr->numtextures; i++)
    {
