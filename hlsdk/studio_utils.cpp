...
void StudioModel::UploadTexture(mstudiotexture_t *ptexture, byte *data, byte *pal, int texnum, bool import )
{
...
bool StudioModel::LoadModel( char *modelname ) throw()
{
	FILE *fp;
	...
	if (phdr->textureindex > 0)
	{
		...
		{
			char str[MAX_PATH+22];
			sprintf(str, "error on openening %s\n", modelname );
			throw( str );
			return false;
		}
	}
...
bool StudioModel::SaveModel( char *modelname ) throw()
{
	FILE				*fp;
	int					i;
	studiohdr_t			*pHdr;
...
bool StudioModel::LoadTextures( char *modelname ) throw()
{
	FILE *fp;
...
bool StudioModel::LoadDemandSequence( char *modelname, int index ) throw()
{
	FILE *fp;
...
bool StudioModel::Init( char *modelname ) throw()
{ 
	try
	{
		...
		if(memcmp(CoreFile,"IDST",4)!=0)
		{
			...
			char str[256];
			sprintf( str, "ERROR: %s isn't is a valid HL model file.", modelname);
			throw( str );
			return false;
		}
	}
	...
	if ( !m_pstudiohdr )
	{
		throw( "ERROR: Couldn't open the Model right." );
		return false;
	}
	// preload textures
	if (m_pstudiohdr->numtextures == 0)
	{
		...
		if ( !m_ptexturehdr )
		{
			throw( "ERROR: Couldn't open the Textures right." );
			return false;
		}
	}
	else
	...
	if (m_pstudiohdr->numseqgroups > 1)
	{
		for (int i = 1; i < m_pstudiohdr->numseqgroups; i++)
		{
			...
			try
			{
				LoadDemandSequence( seqgroupname, i );
			...
			if ( !m_panimhdr[i] )
			{
				throw( "ERROR: Couldn't open the Sequences right." );
				return false;
			}
		}
	}
...
mstudioseqdesc_t *StudioModel::GetSequence( char *name )
{
...
mstudioevent_t *StudioModel::GetEvent( char *seqname )
{
	mstudioseqdesc_t *pseqdescs = (mstudioseqdesc_t *) ((byte *) m_pstudiohdr + m_pstudiohdr->seqindex);
	for ( int i = 0; i < m_pstudiohdr->numseq; i++ )
	{
...
void StudioModel::SetFrame( int frame )
{
	m_frame = (float)frame;
}
...
void StudioModel::SetSelHitbox( int index )
{
	m_selhitbox = index;
}

void StudioModel::SetSelBone( int index )
{
	m_selbone = index;
}
