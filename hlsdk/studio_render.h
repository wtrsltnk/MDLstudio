...
public:
    ...
    mstudioseqdesc_t *GetSequence(const char *name);
    mstudioevent_t *GetEvent(const char *seqname);
    ...
    long GetPolyCount();
    mstudiotexture_t *GetMaterial(const char *name);

    // EXPORT FUNCTIES
    bool BMP_ExportTexture(const char *Texname, const char *Filename);
    bool BMP_ImportTexture(const char *Texname, const char *Filename);
    bool BMP_WriteTexture(byte *pBitData, byte *pColorData, const char *sFilename, mstudiotexture_t *pTexture);
    bool BMP_ReadTexture(byte *pBitData, byte *pColorData, const char *sFilename, mstudiotexture_t *pTexture);
    bool SMD_ExportSequence(const char *Seqname, const char *Filename);
    void SMD_WriteFrame(FILE *smdfile, int frame, mstudioanim_t *panim, mstudioseqdesc_t *pseqdesc);
    void SMD_WriteNodes(FILE *smd);
    bool SMD_GenerateReferences(int Bodyindex, const char *Refname, const char *Filename);
...
private:
    ...
    bool LoadTextures(const char *modelname);
    bool LoadDemandSequence(const char *modelname, int index);
