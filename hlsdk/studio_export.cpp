...
bool StudioModel::BMP_ExportTexture(
    const char *Texname,
    const char *Filename)
{
    byte *pColorData;
...
bool StudioModel::BMP_WriteTexture(
    byte *pBitData,
    byte *pColorData,
...
bool StudioModel::SMD_ExportSequence(
    const char *Seqname,
    const char *filename)
{
    FILE *smdfile;
...
*/
            ...
            if (!smdfile)
            {
                ...
                sprintf(str, "ERROR: Can't write %s\r\n", smdfilename);
                throw(str);
                return false;
            }
...
==============================================================
*/
void StudioModel::SMD_WriteFrame(
    FILE *smdfile,
    int frame,
    mstudioanim_t *panim,
    mstudioseqdesc_t *pseqdesc)
...
void StudioModel::SMD_WriteNodes(
    FILE *smd)
{
    int i;
    studiohdr_t *phdr;
...
==============================================================
*/
bool StudioModel::SMD_GenerateReferences(
    int Bodyindex,
    const char *Refname,
    const char *Filename)
{
                    ...
                    if (ii < 0)
                    {
                        ...
                        for (; ii > 0; ii--, ptris += 4, iii++)
                        {
                            ...
                            switch (iii)
                            {
                                ...
                                default:
                                    pC = pB;
                                    pB = ptris; // pA = no change, pC = pB , pB = new
                                    //fprintf(smdfile,"iii %i ",iii);
                                    SMD_WriteTriangle(smdfile, pmodel, ptexture, pA, pB, pC);
                            }
                        }
                        ...
                        for (; ii > 0; ii--, ptris += 4, iii++)
                        {
                            ...
                            switch (iii)
                            {
                                ...
                                default:
                                    ...
                                    {
                                        pA = pB;
                                        pB = ptris; // pA = pC, pB = new , pC = no change
                                        //fprintf(smdfile,"iii %i ",iii);
                                        SMD_WriteTriangle(smdfile, pmodel, ptexture, pA, pB, pC);
                                    }
                                    else
                                    {
                                        pA = pC;
                                        pC = ptris; // pA = pC, pB no change, pC = new
                                        //fprintf(smdfile,"iii %i ",iii);
                                        SMD_WriteTriangle(smdfile, pmodel, ptexture, pA, pB, pC);
                                    }
                            }
...
void StudioModel::SMD_WriteTriangle(
    FILE *smdfile,
    mstudiomodel_t *pmodel,
    mstudiotexture_t *ptexture,
    short *pA,
    ...
    short *pC)
{
    ...
    fprintf(smdfile, "%3i %f %f %f %f %f %f %f %f\n", pvertbone[pA[0]], refvertex[0], refvertex[1], refvertex[2], refnormal[0], refnormal[1], refnormal[2], pA[2] * s, 1 - (pA[3] * t));
    ...
    VectorNormalize(refnormal);
    //fprintf( smdfile, "%3i %8.4f %8.4f %8.4f %8.4f %8.4f %8.4f %8.4f %8.4f\n"
    fprintf(smdfile, "%3i %f %f %f %f %f %f %f %f\n", pvertbone[pB[0]], refvertex[0], refvertex[1], refvertex[2], refnormal[0], refnormal[1], refnormal[2], pB[2] * s, 1 - (pB[3] * t));
    ...
    VectorNormalize(refnormal);
    //fprintf( smdfile, "%3i %8.4f %8.4f %8.4f %8.4f %8.4f %8.4f %8.4f %8.4f\n"
    fprintf(smdfile, "%3i %f %f %f %f %f %f %f %f\n", pvertbone[pC[0]], refvertex[0], refvertex[1], refvertex[2], refnormal[0], refnormal[1], refnormal[2], pC[2] * s, 1 - (pC[3] * t));
}
