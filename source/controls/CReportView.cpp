...
/////////////////////////////////////
CReportView::CReportView( HINSTANCE hInstance, 
					 HWND hParent,
					 int nX, int nY , 
...
CReportView::CReportView( HINSTANCE hInstance, HWND hParent ) : CControl( hInstance, hParent )
{
}
...
void CReportView::create( DWORD ex_style )
{
...
void CReportView::create( DWORD ex_style, DWORD styles )
{
	m_hWnd = CreateWindowEx( ex_style,
		 WC_LISTVIEW, 
		 "Caption", 
...
void CReportView::setImageList( UINT wBMID, int cx, int cy, int iNoImg )
{
...
void CReportView::setExStyle( DWORD style )
{
	ListView_SetExtendedListViewStyleEx( m_hWnd, NULL, style );
}


void CReportView::reset( void )
{
	int i = 0;
	int result;
...
void CReportView::addColumn( string str, int iColNo, int width )
{
...
void CReportView::setColumnWidth( int iColNo, int nWidth )
{
	SendMessage( m_hWnd, 
		LVM_SETCOLUMNWIDTH, 
		( WPARAM ) iColNo, 
...
void CReportView::addItem( string str, int iRowNo, int iImage )
{
	LV_ITEM lvi;
...
void CReportView::addSubItem( string s, int iRowNo, int iSubItem )
{
...
int CReportView::getSelectedIndex( void )
{
	return ListView_GetSelectionMark( m_hWnd );
}

string CReportView::getItemText( int row )
{
	char text[255];
...
string CReportView::getSubItemText( int row, int sub )
{
	char text[255];
