...
CFrame::CFrame(
    ...
    HWND hParent,
    int nX, int nY,
    int nWidth, int nHeight, int id, int optMax)
    : CControl(hInstance,
               hParent,
               nX, nY,
               nWidth, nHeight, id)
    ...
    int nHeight)
{
    if (optCount < 10)
    {
        m_hOptions[optCount++] = CreateWindowEx(
            WS_EX_WINDOWEDGE,
            "BUTTON",
            text.c_str(),
            WS_CHILD | WS_VISIBLE |
...
std::string CFrame::getOptionText(
    int index)
{
    if (index > optCount)
