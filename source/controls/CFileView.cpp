...
/////////////////////////////////////
CFileView::CFileView( HINSTANCE hInstance, 
					 HWND hParent,
					 int nX, int nY , 
...
CFileView::CFileView( HINSTANCE hInstance, HWND hParent ) : CControl( hInstance, hParent )
{
}
...
void CFileView::create( DWORD ex_style )
{
...
void CFileView::create( DWORD ex_style, DWORD styles )
{
	m_hWnd = CreateWindowEx( ex_style,
		 WC_LISTVIEW, 
		 "Caption", 
...
void CFileView::initFiles( string strFilePath )
{
	HANDLE hFind;
	WIN32_FIND_DATA fd;
...
void CFileView::setImageList( UINT wBMID, int cx, int cy, int iNoImg )
{
...
DWORD CFileView::getStyle( void )
{
	return GetWindowLong( m_hWnd, GWL_STYLE );
}

void CFileView::reset( void )
{
	int i = 0;
	int result;
...
void CFileView::addColumn( string str, int iColNo, int width )
{
...
void CFileView::setColumnWidth( int iColNo, int nWidth )
{
	SendMessage( m_hWnd, 
		LVM_SETCOLUMNWIDTH, 
		( WPARAM ) iColNo, 
...
int CFileView::getColumnWidth( int iColNo )
{
...
void CFileView::addItem( string str, int iRowNo, int iImage )
{
	LV_ITEM lvi;
...
void CFileView::addSubItem( string s, int iRowNo, int iSubItem )
{
...
string CFileView::getItemText( int row )
{
	char text[255];
...
string CFileView::getSubItemText( int row, int sub )
{
	char text[255];
