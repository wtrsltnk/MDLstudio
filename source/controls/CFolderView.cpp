...
void CFolderView::initMycomputer()
{
    initIcons();
...
std::string CFolderView::initPath(
    WPARAM wParam,
    LPARAM lParam)
{
    HANDLE hFind;
...
void CFolderView::clearChilderen(
    HTREEITEM hItem)
{
...
void CFolderView::setButtonState(
    HTREEITEM hItem,
...
bool CFolderView::hasSubdirectory(
    const std::string &strPathName)
{
    HANDLE hFind;
    WIN32_FIND_DATA fd;
    bool bResult = false;
    std::string strFileSpec, strFileName;
...
bool CFolderView::isDriveNode(
    HTREEITEM hItem)
{
    return (getParent(hItem) == NULL) ? true : false;
}

void CFolderView::addDummyNode(
    HTREEITEM hItem)
{
    addItem("searching...", hItem, 1, 2);
}

std::string CFolderView::getItemPath(
    HTREEITEM hItem)
{
    HTREEITEM hHigh;
...
std::string CFolderView::getSelItemPath()
{
    HTREEITEM hHigh;
    std::string result, temp;
...
HTREEITEM CFolderView::getSelection()
{
    return TreeView_GetSelection(m_hWnd);
}

TVITEM CFolderView::getItem(
    HTREEITEM hitem)
{
    TVITEM item;
    char text[255];
...
std::string CFolderView::getItemText(
    HTREEITEM hItem)
{
    TVITEM item;
    char text[255];
...
HTREEITEM CFolderView::getParent(
    HTREEITEM hitem)
{
    return TreeView_GetParent(m_hWnd, hitem);
}

HTREEITEM CFolderView::getChild(
    HTREEITEM hitem)
{
    return TreeView_GetChild(m_hWnd, hitem);
}

void CFolderView::expand(
    HTREEITEM hItem)
{
