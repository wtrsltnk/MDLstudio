...
bool CCManager::initGui( StudioModel *pModel )
{
	bool result;
	int i, j;
	char str[64];
	...
	result = m_p3DView->init( m_pModel );
	if ( result )
	{
		// REFERENCE
		studiohdr_t *hdr = m_pModel->getStudioHeader ();

		sprintf( str, "References (%i)", hdr->numbodyparts );
		...
		// SKIN
		sprintf( str, "Skins (%i)", hdr->numskinfamilies );
		...
		// CONTROLLER
		sprintf( str, "Controllers (%i)", hdr->numbonecontrollers );
		...
		// SEQUENCE
		sprintf( str, "Sequences (%i)", hdr->numseq );
		...
		sprintf( str, "Materials (%i)", hdr->numskinref );
		...
		sprintf( str, "Bones (%i)", hdr->numbones );
...
void CCManager::clearGui( void )
{
	...
	m_pSpeed->init(0, 100);
	m_pSpeed->setSel( settings.speed * 10 );
	sprintf( buffer, "Animation speed : %i", m_pSpeed->getSel() );
...
void CCManager::ItemSelected( WPARAM wParam, LPARAM lParam )
{
	int i;
	studiohdr_t *hdr;
	char buffer[255];
...
int CCManager::getIndex( string str )
{
	...
	for ( int i = strlen(temp)-1; i > 0; i-- )
	{
		...
		{
            result = new char[strlen(temp)-i+1];
			for ( int j = 0; j < strlen(temp)-i; j++ )
			{
				result[j] = temp[j+i+1];
			}
            result[strlen(temp)-i] += '\0';
			return atoi(result);
		}
	}
	return 0;
...
void CCManager::SliderScoll( HWND hWnd, WPARAM wParam, LPARAM lParam )
{
	char buffer[255];
...
string CCManager::getTreeSelectedText( void )
{
	return m_pPanel->getItemText( m_pPanel->getSelection() );
}

string CCManager::getTreeParentText( void )
{
...
void CCManager::setStatus( char *text, ... )
{
    va_list argp;
    char result[256];
...
void CCManager::centerview( void )
{
	m_p3DView->centerview();
}

void CCManager::vmodelview( void )
{
	m_p3DView->vmodelview();
}

void CCManager::statusPan( void )
{
	char str[255];
