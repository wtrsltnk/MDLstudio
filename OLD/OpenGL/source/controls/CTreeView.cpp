...
CTreeView::CTreeView( HINSTANCE hInstance, HWND hParent ) : CControl( hInstance, hParent )
{
}
...
void CTreeView::create( DWORD ex_style, DWORD styles )
{
	m_hWnd = CreateWindowEx( ex_style,
		 WC_TREEVIEW, 
		 "Caption", 
...
void CTreeView::setImageList( UINT wBMID, int cx, int cy, int iNoImg )
{
...
HTREEITEM CTreeView::addItem( string str, HTREEITEM hParent, int iImage )
{
...
HTREEITEM CTreeView::getSelection( void )
{
	return TreeView_GetSelection( m_hWnd );
}

TVITEM CTreeView::getItem( HTREEITEM hitem )
{
	TVITEM		item;
	char		text[255];
...
string CTreeView::getItemText( HTREEITEM hItem )
{
	TVITEM item;
	char text[ 255 ];
	item.hItem		=	hItem;
