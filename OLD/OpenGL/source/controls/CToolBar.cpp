...
CToolBar::CToolBar( HINSTANCE hInstance, HWND hParent, int id ) : CControl( hInstance, 
					 hParent,
					 0, 0 , 
					 0 ,0, id )
{
...
void CToolBar::create( UINT wBMID, int iNumBut, int iButWidth, int iButHeight )
{
	m_hWnd = CreateToolbarEx( m_hParent, 
		WS_CHILD | WS_VISIBLE |
...
Add toolbar button
*/
void CToolBar::addButton( char *caption, int idCommand, int iBitmap )
{
	TBBUTTON tb;
	...
	tb.dwData = 0L;
	tb.iString = SendMessage( m_hWnd, TB_ADDSTRING, 0, ( LPARAM ) ( LPSTR ) caption );
	tb.iBitmap = iBitmap;
...
Remove Button
*/
bool CToolBar::remButton( int iButNo )
{
...
Add seperator
*/
void CToolBar::addSeperator( void )
{
	TBBUTTON tb;
