...
void StudioModel::DrawUVMap( int skin, int nWidth, int nHeight )
{
	...
	if (m_pstudiohdr)
	{
		mstudiotexture_t *ptextures = (mstudiotexture_t *) ((byte *) m_pstudiohdr + m_pstudiohdr->textureindex);
		w = (float) ptextures[skin].width * settings.zoom;
		h = (float) ptextures[skin].height * settings.zoom;
		...
		glRectf (x - 1, y - 1, x  + w + 1, y + h + 1);

		glEnable (GL_TEXTURE_2D);
		glColor4f (1.0f, 1.0f, 1.0f, 1.0f);
		glBindTexture (GL_TEXTURE_2D, skin+3);
...
//
	...
	for (j = 0; j < m_pmodel->nummesh; j++) 
	{
		...
		polycount+= pmesh->numtris;

		s = 1.0/(float)ptexture[pskinref[pmesh->skinref]].width;
		t = 1.0/(float)ptexture[pskinref[pmesh->skinref]].height;

		glBindTexture( GL_TEXTURE_2D, ptexture[pskinref[pmesh->skinref]].index );
		if ( ptexture[pskinref[pmesh->skinref]].flags == 64 )
		{
			// Transparent
		}
		else if ( ptexture[pskinref[pmesh->skinref]].flags == 32 )
		{
			// Additive
		}
		else
